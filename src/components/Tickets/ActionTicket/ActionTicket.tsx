...
export const ActionTicket: React.FC<IActionTicket> = ({ type = 'normal' }) => {
  ...
  return (
    <ContainerActionTicket className={type === 'normal' ? '' : 'chairs'}>
      {eventTicket && (
        type && type === 'normal' && (
      ...
      {ticket ? (
        <ComponentTicket data={ticket} type={type} />
      ) : (
        <Tickets />
      )}
      {eventTicket && eventTicket.tiposDeIngresso && eventTicket.tiposDeIngresso.length > 0 ? (
        <>
          <div className={`btn-submit ${ticket ? 'active' : ''}`}>
            <div className="info-purchase">
              <p className="text-dark">
                {ticketsPurchase && ticketsPurchase.length}
                {' '}
                selecionados
              </p>
            ...
            <Button disabled={ticketsPurchase && ticketsPurchase.length > 0 ? false : true} onClick={handleShowModal}>Comprar</Button>
          </div>
          {ticketsPurchase && ticketsPurchase.length > 0 && (
            <div className="btn-submit btn-submit-mobile">
              <div className="info-purchase">
                <p className="text-dark">
                  {ticketsPurchase && ticketsPurchase.length}
                  {' '}
        ...
        <div className="empty">
          <p>Aguarde o inicio das vendas</p>
        </div>
      )}
    </ContainerActionTicket>
  );
};
